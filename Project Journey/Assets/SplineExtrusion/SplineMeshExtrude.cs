...
public class SplineMeshExtrude : MonoBehaviour
{
    ...
    private Mesh GenerateMesh()
    {
        ...
        // Checking how many intervals there are per spline position. Thanks Robert
        //Debug.Log(/*float intervalsInSubSegment = */positions.Length / spline.Count);
        //Debug.Log(/*float intervalsInSubSegment = */spline.Count / positions.Length);
        ...
        // Do not create a mesh for the first and last segment of the spline I.F.

        // TODO: Calculate how many "positions" between spline[^1] and spline[^2]
        //Debug.Log("Containers " + splineContainer.Splines.Count);
        splineContainer.Splines[1].Add(splineContainer.Splines[0][^2]);
        splineContainer.Splines[1].Add(splineContainer.Splines[0][^1]);
        
        //bool successEnd = SplineUtil.SampleSplineInterval(splineContainer.Splines[1], transform, extrusionInterval, /**/ out Vector3[] positionsEnd /**/, out Vector3[] tangentsEnd, out Vector3[] upVectorsEnd);

        Vector3[] positionsEnd = SplineUtil.SampleSplineInterval(splineContainer.Splines[1], transform, extrusionInterval);
        //Debug.Log("End positions " + positionsEnd.Length);
        
        // TODO: Calculate how many "positions" between spline[0] and spline[1]
        splineContainer.Splines[2].Add(splineContainer.Splines[0][0]);
        splineContainer.Splines[2].Add(splineContainer.Splines[0][1]);
        
        Vector3[] positionsStart = SplineUtil.SampleSplineInterval(splineContainer.Splines[2], transform, extrusionInterval);
        //bool successStart = SplineUtil.SampleSplineInterval(splineContainer.Splines[2], transform, extrusionInterval, out Vector3[] positionsStart, out Vector3[] tangentsStart, out Vector3[] upVectorsStart);
        
        //TODO: This isnt quite right yet.... I.F.
        for (int i = positionsStart.Length; i < positions.Length - positionsEnd.Length; i++)
        {
